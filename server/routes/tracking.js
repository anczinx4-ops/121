...
router.get('/batch/:eventId', async (req, res) => {
  try {
    ...
    const batchesResult = await kaleidoService.getAllBatches();
    if (!batchesResult.success) {
      return res.status(500).json({
        success: false,
        error: 'Failed to get batches from Kaleido network'
      });
    }
    ...
    for (const batch of batches) {
      const eventsResult = await kaleidoService.getBatchEvents(batch.batchId);
    ...
    const eventsResult = await kaleidoService.getBatchEvents(targetBatch.batchId);
    if (!eventsResult.success) {
      return res.status(500).json({
        success: false,
        error: 'Failed to get batch events from Kaleido network'
      });
    }
...
router.get('/stats/:batchId', async (req, res) => {
  try {
    ...
    const eventsResult = await kaleidoService.getBatchEvents(batchId);
    if (!eventsResult.success) {
      return res.status(500).json({
        success: false,
        error: 'Failed to get batch events from Kaleido network'
    ...
    const enhancedBatches = await Promise.all(
      batches.map(async (batch) => {
        ...
        return {
          ...
          lastUpdated: events.length > 0 ? Math.max(...events.map(event => event.timestamp * 1000)) : 0,
